		...
		<div id="list-gerente" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="username" title="${message(code: 'gerente.username.label', default: 'Username')}" />
					
						<g:sortableColumn property="password" title="${message(code: 'gerente.password.label', default: 'Password')}" />
				...
				<g:each in="${gerenteInstanceList}" status="i" var="gerenteInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${gerenteInstance.id}">${fieldValue(bean: gerenteInstance, field: "username")}</g:link></td>
					
						<td>${fieldValue(bean: gerenteInstance, field: "password")}</td>
					
						<td>${fieldValue(bean: gerenteInstance, field: "nome")}</td>
